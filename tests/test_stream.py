"""Tests for fastpack streaming features (v0.4.0)."""
...
class TestPackTo:
    ...
    def test_pack_to_bytesio(self):
        buffer = io.BytesIO()
        bytes_written = fastpack.pack_to({"name": "Ana"}, buffer)

        assert bytes_written > 0
        buffer.seek(0)
        assert fastpack.unpack(buffer.read()) == {"name": "Ana"}

    def test_pack_to_file(self):
        with tempfile.NamedTemporaryFile(delete=False) as f:
            bytes_written = fastpack.pack_to([1, 2, 3], f)
            temp_path = f.name

        assert bytes_written > 0

        with open(temp_path, "rb") as f:
            result = fastpack.unpack(f.read())
    ...
    def test_pack_to_returns_correct_size(self):
        ...
        bytes_written = fastpack.pack_to(data, buffer)
        packed = fastpack.pack(data)
...
class TestUnpackFrom:
    ...
    def test_unpack_from_bytesio(self):
        data = fastpack.pack({"id": 123})
        buffer = io.BytesIO(data)

        result = fastpack.unpack_from(buffer)
        assert result == {"id": 123}

    def test_unpack_from_file(self):
        with tempfile.NamedTemporaryFile(delete=False) as f:
            f.write(fastpack.pack("hello"))
            temp_path = f.name

        with open(temp_path, "rb") as f:
            result = fastpack.unpack_from(f)
...
class TestPackStream:
    ...
    def test_pack_stream_list(self):
        ...
        bytes_written = fastpack.pack_stream(items, buffer)

        assert bytes_written > 0
        buffer.seek(0)
        result = list(fastpack.unpack_stream(buffer))
        assert result == items
    ...
    def test_pack_stream_generator(self):
        ...
        buffer = io.BytesIO()
        fastpack.pack_stream(gen(), buffer)
    ...
    def test_pack_stream_empty(self):
        buffer = io.BytesIO()
        bytes_written = fastpack.pack_stream([], buffer)
    ...
    def test_pack_stream_mixed_types(self):
        ...
        fastpack.pack_stream(items, buffer)
    ...
    def test_pack_stream_to_file(self):
        ...
        with open(temp_path, "rb") as f:
            result = list(fastpack.unpack_stream(f))
...
class TestUnpackStream:
    ...
    def test_unpack_stream_returns_iterator(self):
        buffer = io.BytesIO()
        fastpack.pack_stream([1, 2, 3], buffer)
        buffer.seek(0)

        result = fastpack.unpack_stream(buffer)
    ...
    def test_unpack_stream_lazy(self):
        ...
        iterator = fastpack.unpack_stream(buffer)
    ...
    def test_unpack_stream_partial_length_raises(self):
        ...
        with pytest.raises(ValueError, match="reading length"):
            list(fastpack.unpack_stream(buffer))
...
class TestPackMany:
    ...
    def test_pack_many_empty(self):
        data = fastpack.pack_many([])
        assert data == b""
        assert fastpack.unpack_many(data) == []

    def test_pack_many_single(self):
        data = fastpack.pack_many(["hello"])
        result = fastpack.unpack_many(data)
        assert result == ["hello"]
...
class TestIterUnpack:
    ...
    def test_iter_unpack_basic(self):
        data = fastpack.pack_many([1, 2, 3])
        result = list(fastpack.iter_unpack(data))
        assert result == [1, 2, 3]

    def test_iter_unpack_is_lazy(self):
        data = fastpack.pack_many(list(range(100)))
        iterator = fastpack.iter_unpack(data)
    ...
    def test_iter_unpack_partial_length_raises(self):
        ...
        with pytest.raises(ValueError, match="reading length"):
            list(fastpack.iter_unpack(b"\x00\x00"))
